...
robert_vertical = output  
for i in range(img_h):
    for j in range(img_w):
            if robert_vertical[i][j] > 255:
               robert_vertical[i][j] = 255
            elif robert_vertical[i][j] < 0:
                robert_vertical[i][j] = 0
...
robert_horizontal = output  
for i in range(img_h):
    for j in range(img_w):
            if robert_horizontal[i][j] > 255:
               robert_horizontal[i][j] = 255
            elif robert_horizontal[i][j] < 0:
                robert_horizontal[i][j] = 0

robert_merged = robert_horizontal + robert_vertical
for i in range(img_h):
    for j in range(img_w):
            if robert_merged[i][j] > 255:
               robert_merged[i][j] = 255
            elif robert_merged[i][j] < 0:
                robert_merged[i][j] = 0

img = img + robert_merged
...
show_images([img1,robert_vertical], 
            ['input', 'robert vertical'])
show_images([robert_horizontal, robert_merged], 
            ['robert horizontal', 'robert merged'])
